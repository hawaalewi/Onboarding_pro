...
export const uploadProfilePhoto = async (req, res) => {
  try {
    ...
    // Verify user is a job seeker or organization
    const user = await User.findById(userId);
    if (!user || (user.accountType !== 'job_seeker' && user.accountType !== 'organization')) {
      return res.status(403).json({
        success: false,
        message: 'Access denied. Valid account required.'
      });
    }
    ...
    } else if (user.companyInfo?.logoUrl) {
      // Delete old logo
      try {
        const urlParts = user.companyInfo.logoUrl.split('/');
        const publicId = urlParts[urlParts.length - 1].split('.')[0];
        const folder = 'onboarding_project/profile_photos';
        await cloudinary.uploader.destroy(`${folder}/${publicId}`);
      } catch (error) {
    ...
    try {
      ...
      // Update user profile based on account type
      if (user.accountType === 'job_seeker') {
        ...
        user.companyInfo.logoUrl = result.secure_url;
      }
